...
function set(path, value, to) {
  var schema = to;  // a moving reference to internal objects within obj
...
if (save) {
  ...
  for (let i = 0; i < missingPaths.length; i++) {
    const paths = missingPaths[i] ?? [];
    // const paths = el.split('.');
    for (const path of paths) {
      const d = getValueByPath(referenceData, path);
      set(path, d, checkData[0]);
    }
  }
  const data = JSON.stringify(checkData[0], null, 2);
  const filename = checkFilenames[0].split('.')[0] + '.missing.json';
